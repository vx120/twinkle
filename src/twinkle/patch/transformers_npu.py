...
class NPUQwen3VLMoeTextExperts(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, routing_weights: torch.Tensor, router_indices: torch.Tensor):
        ...
        if self.training:
            ...
            batch_idx = torch.arange(num_tokens, device=routing_weights.device).unsqueeze(1).expand(-1, top_k)
...
def _patch_forward_if_exists(module, class_names, forward_fn):
    for class_name in class_names:
        cls = getattr(module, class_name, None)
        if cls is not None:
            cls.forward = forward_fn
            return True
    logger.debug('Skip NPU patch for %s: none of %s exist', module.__name__, class_names)
    return False


def _patch_symbol_if_exists(module, symbol_name, value):
    if hasattr(module, symbol_name):
        setattr(module, symbol_name, value)
        return True
    logger.debug('Skip NPU patch for %s.%s: symbol not found', module.__name__, symbol_name)
    return False


_patch_forward_if_exists(modeling_qwen2, ['Qwen2RMSNorm'], rms_norm_forward_npu)
_patch_forward_if_exists(modeling_qwen2, ['Qwen2MLP'], silu_forward_npu)
_patch_symbol_if_exists(modeling_qwen2, 'apply_rotary_pos_emb', apply_rotary_pos_emb_npu)

_patch_forward_if_exists(
    modeling_qwen2_5_vl,
    ['Qwen2_5_VLTextRMSNorm', 'Qwen2RMSNorm'],
    rms_norm_forward_npu,
)
_patch_forward_if_exists(
    modeling_qwen2_5_vl,
    ['Qwen2_5_VLMLP', 'Qwen2_5_VLTextMLP'],
    silu_forward_npu,
)
_patch_symbol_if_exists(modeling_qwen2_5_vl, 'apply_rotary_pos_emb', apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3, ['Qwen3RMSNorm'], rms_norm_forward_npu)
_patch_forward_if_exists(modeling_qwen3, ['Qwen3MLP'], silu_forward_npu)
_patch_symbol_if_exists(modeling_qwen3, 'apply_rotary_pos_emb', apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3_moe, ['Qwen3MoeRMSNorm'], rms_norm_forward_npu)
_patch_forward_if_exists(modeling_qwen3_moe, ['Qwen3MoeSparseMoeBlock'], qwen3_moe_sparse_moe_block_forward_npu)
_patch_symbol_if_exists(modeling_qwen3_moe, 'apply_rotary_pos_emb', apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3_vl, ['Qwen3VLTextRMSNorm'], rms_norm_forward_npu)
_patch_forward_if_exists(modeling_qwen3_vl, ['Qwen3VLTextMLP'], silu_forward_npu)

_patch_symbol_if_exists(modeling_qwen3_vl_moe, 'Qwen3VLMoeTextSparseMoeBlock', NPUQwen3VLMoeTextSparseMoeBlock)
_patch_forward_if_exists(modeling_qwen3_vl_moe, ['Qwen3VLMoeTextRMSNorm'], rms_norm_forward_npu)
_patch_symbol_if_exists(modeling_qwen3_vl_moe, 'apply_rotary_pos_emb', apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3_next, ['Qwen3NextSparseMoeBlock'], qwen3_next_sparse_moe_block_forward_npu)
_patch_forward_if_exists(modeling_qwen3_next, ['Qwen3NextRMSNormGated'], qwen3_next_rms_norm_forward_gated_npu)
_patch_forward_if_exists(modeling_qwen3_next, ['Qwen3NextRMSNorm'], qwen3_next_rms_norm_forward_npu)
_patch_symbol_if_exists(modeling_qwen3_next, 'apply_rotary_pos_emb', qwen3_next_apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3_5, ['Qwen3_5RMSNormGated'], qwen3_next_rms_norm_forward_gated_npu)
_patch_forward_if_exists(modeling_qwen3_5, ['Qwen3_5RMSNorm'], qwen3_next_rms_norm_forward_npu)
_patch_symbol_if_exists(modeling_qwen3_5, 'apply_rotary_pos_emb', qwen3_next_apply_rotary_pos_emb_npu)

_patch_forward_if_exists(modeling_qwen3_5_moe, ['Qwen3_5MoeExperts'], qwen3_5_moe_experts_forward_npu)
_patch_forward_if_exists(modeling_qwen3_5_moe, ['Qwen3_5MoeRMSNormGated'], qwen3_next_rms_norm_forward_gated_npu)
_patch_forward_if_exists(modeling_qwen3_5_moe, ['Qwen3_5MoeRMSNorm'], qwen3_next_rms_norm_forward_npu)
_patch_symbol_if_exists(modeling_qwen3_5_moe, 'apply_rotary_pos_emb', qwen3_next_apply_rotary_pos_emb_npu)
