...
def check_vllm_ascend_before_server_launch():
    ...
    def _is_ascend_soc_version_a2_v011_local():
        ...
        if 220 <= soc_version <= 225:
            current_soc = AscendSocVersion.A2
        elif 250 <= soc_version <= 255:
            current_soc = AscendSocVersion.A3
        else:
            current_soc = AscendSocVersion.UNDEFINED

        return current_soc == AscendSocVersion.A2
    ...
    def _is_ascend_soc_version_a2_v013_local():
        ...
        soc_version = torch_npu.npu.get_soc_version()
        if 220 <= soc_version <= 225:
            current_device_type = AscendDeviceType.A2
        elif 250 <= soc_version <= 255:
            current_device_type = AscendDeviceType.A3
        elif 200 <= soc_version <= 205:
            current_device_type = AscendDeviceType._310P
        elif soc_version == 260:
            current_device_type = AscendDeviceType.A5
        else:
            raise RuntimeError(f"Can not support soc_version: {soc_version}.")

        return current_device_type == AscendDeviceType.A2
...
if Torch.is_torch_npu_available():
    import vllm
    from packaging import version

    _vllm_version = version.parse(vllm.__version__)
    if version.parse("0.13.0") <= _vllm_version <= version.parse("0.14.0"):
        from vllm.model_executor.layers.fused_moe import FusedMoE

        patch_vllm013_rotary_emb()
        FusedMoE.weight_loader = vllm_v013_weight_loader_method_wrapper(FusedMoE.weight_loader)

    twinkle_npu_enable_a2_patch_vllm_ascend_mc2 = bool(
        int(os.getenv("TWINKLE_NPU_ENABLE_A2_PATCH_VLLM_ASCEND_MC2", "1"))
    )
    if twinkle_npu_enable_a2_patch_vllm_ascend_mc2:
        if version.parse("0.13.0") <= _vllm_version <= version.parse("0.14.0"):
        ...
        elif version.parse("0.11.0") <= _vllm_version < version.parse("0.13.0"):
            from vllm_ascend.ops.linear_op import SequenceRowParallelOp
